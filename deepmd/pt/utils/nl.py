...
def neighbor_list_and_relative_vec(
    ...
    distances, edge_vectors = None, None
    if NL == "vesin":
        ...
        first_idex, second_idex, distances, edge_vectors, shifts = ase.neighborlist.primitive_neighbor_list(
            "ijdDS",
            pbc,
            temp_cell,
            temp_pos,
    ...
    return edge_index, shifts, cell_tensor, distances, edge_vectors


def compute_neighborlist_(
    data: AtomicDataDict.Type, r_max: float, use_angle: bool = True, **kwargs
) -> AtomicDataDict.Type:
    ...
    if use_angle:
        ...
        for idx in range(AtomicDataDict.num_frames(data)):
            ...
            edge_index, edge_cell_shift, cell, distances, edge_vectors = neighbor_list_and_relative_vec(
            ...
            n_atoms = data_per_frame[AtomicDataDict.POSITIONS_KEY].shape[0]
            graph = Graph([Node(index=idx) for idx in range(n_atoms)])
            
            for ii, jj, img, dist in zip(
                edge_index[0], edge_index[1], edge_cell_shift, distances, strict=True
            ):
                graph.add_edge(ii.item(), jj.item(), distance=dist)
                
            angle_index = torch.tensor(graph.angle_triplets(a_max,edge_index),dtype=torch.long)
            
            
            if len(angle_index.shape) != 2:
                angle_index = torch.tensor([], dtype=torch.long).reshape(0, 3)
            else:
                if n_atoms < max(angle_index[:,0]) + 1:
            ...
            data_per_frame[AtomicDataDict.ANGLE_INDEX_KEY] = angle_index
            if (
