...
def _collate_lmdb_mixed_batch(batch: list[FrameDict]) -> BatchDict:
    ...
    Atom-wise fields are concatenated across frames. Two extra tensors keep the
    frame structure:

    - ``batch`` maps each flattened atom to its frame id.
    - ``ptr`` stores atom prefix sums, shape ``[nframes + 1]``.
    """
...
def make_lmdb_mixed_batch_collate(
    ...
    When ``graph_config`` is supplied, the collate function also precomputes
    flat graph tensors consumed by DPA3/RepFlow flat forward.
    """
...
def _collate_lmdb_batch(batch: list[FrameDict]) -> BatchDict:
    ...
    If a caller accidentally feeds mixed-nloc frames here, fall back to the
    flattened mixed-batch representation.
    """
    if len(batch) > 1:
