...
def _collate_lmdb_mixed_batch(batch: list[FrameDict]) -> BatchDict:
    """Collate mixed-nloc frames into flat atom-wise tensors."""
...
def _collate_lmdb_batch(batch: list[FrameDict]) -> BatchDict:
    ...
    All frames in the batch should have the same nloc (enforced by
    SameNlocBatchSampler when mixed_batch=False). If mixed-nloc frames reach
    this function, they are returned in the flattened mixed-batch format.
    """
    if len(batch) > 1:
