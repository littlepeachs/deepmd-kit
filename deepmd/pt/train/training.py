...
def validate_graph_parallel_config(
    ...
    *,
    ...
    """Validate config constraints for graph-parallel training.

    This intentionally does not query torch.distributed state so it can run
    before process-group initialization and from direct Trainer API paths.
    """
    ...
    if model_type not in {"ener", "standard"}:
        raise ValueError(
            "training.graph_parallel=True currently supports only "
            "model.type 'ener' or 'standard'. "
            f"Got model.type={model_type!r}."
        )

    descriptor_params = model_params.get("descriptor") or {}
    descriptor_type = descriptor_params.get("type")
    if descriptor_type != "dpa3":
        raise ValueError(
            "training.graph_parallel=True currently supports only "
            "descriptor.type 'dpa3'. "
            f"Got descriptor.type={descriptor_type!r}."
        )

    repflow_params = descriptor_params.get("repflow") or {}
    if not repflow_params.get("use_dynamic_sel", False):
        raise ValueError(
            "training.graph_parallel=True currently requires "
            "model.descriptor.repflow.use_dynamic_sel=True."
        )
    if not repflow_params.get("smooth_edge_update", False):
        raise ValueError(
            "training.graph_parallel=True currently requires "
            "model.descriptor.repflow.smooth_edge_update=True."
        )
...
class Trainer:
    def __init__(
        ...
        if use_graph_parallel:
            ...
            if gp_size <= 1:
                raise ValueError(
                    "training.graph_parallel=True requires a graph_parallel "
        ...
        self.use_graph_parallel = use_graph_parallel
        ...
        self._gp_profile = bool(int(os.environ.get("DEEPMD_GP_PROFILE", "0")))
    ...
    def _profile_mark(self, label: str, start: float) -> float:
        """Log a lightweight GP timing mark when requested by env var."""
        if not self._gp_profile:
            return time.perf_counter()
        if torch.cuda.is_available():
            torch.cuda.synchronize()
        now = time.perf_counter()
        log.info(
            "GP_PROFILE rank=%s label=%s elapsed=%.6f",
            self.rank,
            label,
            now - start,
        )
    ...
    def run(self) -> None:
        ...
        def step(_step_id: int, task_key: str = "Default") -> None:
            ...
            profile_t0 = self._profile_mark("get_data", profile_t0)
            if SAMPLER_RECORD:
            ...
            if self.opt_type in ["Adam", "AdamW", "AdaMuon", "HybridMuon"]:
                ...
                pref_lr = cur_lr
                if self.use_moe_ep and self.use_graph_parallel:
                    from deepmd.pt.utils.collective_order import collective_ordering

                    ordering_context = collective_ordering()
                else:
                    ordering_context = nullcontext()
                with ordering_context:
                    ...
                    profile_t0 = self._profile_mark("forward_loss", profile_t0)
                    ...
                    with record_function("loss_backward"):
                        if self.use_moe_ep or self.use_graph_parallel:
                            with self.wrapper.no_sync():
                                backward_loss = loss
                                if self.use_graph_parallel:
                                    # GP force/virial backward mixes GP collectives
                                    # with MoE EP all-to-all nodes. Keep autograd
                                    # execution single-threaded so every rank
                                    # traverses collective nodes in the same order.
                                    ...
                                    backward_loss.backward()
                            profile_t0 = self._profile_mark("backward", profile_t0)
                            # Manual sync: routing experts -> DP group,
                            # shared/non-routing params -> world group.  GP
                            # sums shared gradients because ranks own graph
                            # shards of the same batch, not data-parallel
                            # replicas.
                ...
                with record_function("optimizer_step"):
                    ...
                    with torch.device(DEVICE):
                        self.optimizer.step()
                    self.scheduler.step()
                    profile_t0 = self._profile_mark("optimizer", profile_t0)
        ...
        if self.enable_profiler or self.profiling:
            ...
            if not self.enable_profiler and self.profiling:
                prof.export_chrome_trace(
                    self.profiling_file.split(".")[0] + f"_{self.rank}.json"
                )
                log.info(
                    "The profiling trace has been saved to: "
                    f"{self.profiling_file.split('.')[0]}_{self.rank}.json"
                )
                if self.rank == 0:
                    log.info(
                        "\n%s",
