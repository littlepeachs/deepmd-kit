    ...
    def init_preconditioner(
        ...
        if grad.dim() == 1:
            if not precondition_1d or grad.shape[0] > max_precond_dim:
                ...
                state["GG"].append(
                    torch.zeros(grad.shape[0], grad.shape[0], device=grad.device)  # pylint: disable=no-explicit-dtype
                )
        else:
            if merge_dims:
            ...
            for sh in grad.shape:
                if sh > max_precond_dim:
                    ...
                    state["GG"].append(torch.zeros(sh, sh, device=grad.device))  # pylint: disable=no-explicit-dtype
        ...
        for m in matrix:
            ...
            try:
                _, Q = torch.linalg.eigh(
                    m + 1e-30 * torch.eye(m.shape[0], device=m.device)  # pylint: disable=no-explicit-dtype
                )
            except RuntimeError as e:
                _, Q = torch.linalg.eigh(
                    m.to(torch.float64) + 1e-30 * torch.eye(m.shape[0], device=m.device)  # pylint: disable=no-explicit-dtype
                )
                Q = Q.to(m.dtype)
            Q = torch.flip(Q, [1])
