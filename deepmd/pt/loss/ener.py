...
class EnergyStdLoss(TaskLoss):
    ...
    def _forward_pass_1(self, buffer_id, micro_batch_id):
        if self.stage_id == self.num_stages - 1:
            ...
            extended_virial = torch.einsum('...ij,...ik->...ikj', extended_force, batched_graph['extended_coord'])
            extended_virial = extended_virial.view(list(extended_virial.shape[:-2]) + [9])
            ret_dict = {
                'energy_derv_r': extended_force,
                'energy_derv_c': extended_virial,
            }

            ret_dict = communicate_extended_output_simple(ret_dict, mapping, do_atomic_virial=False)
    ...
    def forward_pp(self,
        ...
        self.pref_v = self.limit_pref_v + (self.start_pref_v - self.limit_pref_v) * coef
